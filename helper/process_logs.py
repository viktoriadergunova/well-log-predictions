...
def process_excel_logs(folder_path, output_dir, nan_value):
    ...
    for file_path in all_files:
        try:
            ...
            # remove trailing/leading in columns
            df.columns = df.columns.str.strip()

            # remove given nan values, with numpy nan
            ...
            # remove entire column if it's all nan to save memory
            df = df.dropna(axis=1, how="all")

            # calculate number of logs for each depth
            log_columns = ["PHIN", "RHOB", "VSH", "DT", "U_pi"]
            available_log_columns = [
                col for col in df.columns if any(col.startswith(log_col) for log_col in log_columns) #r lower
            ]
            ...
            base_name = os.path.basename(file_path).replace(".xlsx", "_processed.xlsx").replace(".csv", "_processed.xlsx")
            ...
            if os.path.exists(output_file_path):
                print(f"Successfully processed and saved logs to {output_file_path}")
            else:
                print(f"Failed to create output file: {output_file_path}")

            print(f"Processed and saved log to {output_file_path}")
